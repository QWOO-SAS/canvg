	  ...
	  c.getContext = function(){
	    ...
	    ['arc', 'arcTo', 'beginPath', 'bezierCurveTo', 'clearRect', 'clip', 'closePath', 'constructor', 'createImageData', 'createLinearGradient', 'createPattern', 'createRadialGradient', 'drawFocusIfNeeded', 'drawImage', 'ellipse', 'fill', 'fillRect', 'fillText', 'getImageData', 'getLineDash', 'isPointInPath', 'isPointInStroke', 'lineTo', 'measureText', 'moveTo', 'putImageData', 'quadraticCurveTo', 'rect', 'resetTransform', 'restore', 'rotate', 'save', 'scale', 'setLineDash', 'setTransform', 'stroke', 'strokeRect', 'strokeText', 'transform', 'translate'].forEach(function(i) {
	      ...
	      ctx[i] = function(){
	        ...
	        var args = [];
	        Array.prototype.slice.call(arguments).forEach(function(j){
	...
	function build(opts) {
	    ...
	    this.setContext = function (ctx, fromMeasure) {
	      ...
	      if (typeof ctx.font != 'undefined') {
	        ctx.font = svg.Font.CreateFont(
	          ...
	          this.style('font-family').value.replace(/[^- \w]/,'')).toString();
	  ...
	  svg.Element.TextElementBase = function (node) {
	    ...
	    this.renderChildren = function (ctx) {
	      ...
	      var text = this.getText();
	      if (dx.length > 1 && dx.length == text.length) {
	        var ox = this.x;
	        var y = this.y;
	        if (ctx.paintOrder == 'stroke') {
	          if (ctx.strokeStyle != '') dx.forEach(function(x, i){ctx.strokeText(text[i], ox + x, y);});
	          if (ctx.fillStyle != '') dx.forEach(function(x, i){ctx.fillText(text[i], ox + x, y);});
	        } else {
	          if (ctx.fillStyle != '') dx.forEach(function(x, i){ctx.fillText(text[i], ox + x, y);});
	          if (ctx.strokeStyle != '') dx.forEach(function(x, i){ctx.strokeText(text[i], ox + x, y);});
	        }
